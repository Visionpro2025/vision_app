...
def _ui_crudas_v2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with c3:
        st.download_button(
            ...
            mime="text/csv", use_container_width=True, key="crudas_descarga"
        )
    return df
...
def _ui_filtradas_v2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ids = st.multiselect("Selecciona id_noticia (filtradas) para acciones",
                         options=ok["id_noticia"].tolist(), key="fil_ids")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        if st.button("📤 Enviar a Cola", use_container_width=True,
                     disabled=not ids, key="fil_cola"):
            _queue_add(ok[ok["id_noticia"].isin(ids)])
            st.success(f"Enviadas {len(ids)} a la cola."); st.rerun()
    with c2:
        if st.button("🔤 Ir a Gematría", use_container_width=True,
                     disabled=not ids, key="fil_gematria"):
            _queue_add(ok[ok["id_noticia"].isin(ids)]); _router_go("gematria")
    with c3:
        if st.button("🌀 Ir a Subliminal", use_container_width=True,
                     disabled=not ids, key="fil_subliminal"):
            _queue_add(ok[ok["id_noticia"].isin(ids)]); _router_go("subliminal")
    with c4:
        if st.button("🗑️ Mover a Papelera", use_container_width=True,
                     disabled=not ids, key="fil_papelera"):
            base = _load_news(NEWS_CSV)
...
def _ui_explorador_v2(df: pd.DataFrame):
    explorer = st.container()  # contenedor estable
    with explorer:
        ...
        fuente = st.selectbox("Fuente", ["Google News (RSS)", "NewsAPI"], index=0, key="exp_fuente")
        q = st.text_input("Consulta (amplia)", _default_query(), key="exp_query")
        n = st.slider("Cantidad a traer por consulta", 20, 100, 60, step=10, key="exp_n")
        window = st.selectbox("Ventana temporal", ["24h","48h","72h"], index=0, key="exp_window")
        days = {"24h":1, "48h":2, "72h":3}[window]

        c1, c2 = st.columns(2)
        with c1:
            if st.button("📥 Traer noticias", use_container_width=True, key="exp_traer"):
        ...
        with c2:
            if st.button("🛰 Traer variedad (multi-consulta)", use_container_width=True, key="exp_variedad"):
                added = _fetch_emergent_now(window_days=days, limit_each=100)
                st.success(f"Variedad agregada: {added}"); st.rerun()

        st.markdown("---")
        st.markdown("### ✍️ Pegar noticias manuales (una por línea)")
        raw = st.text_area("Pega aquí (formato: título | resumen | fuente | url, o 'título — resumen')",
                           height=160, key="exp_paste_raw")
        colp1, colp2, colp3 = st.columns([1,1,1])
        with colp1:
            if st.button("➕ Ingresar a Crudas", use_container_width=True, disabled=not raw.strip(), key="exp_paste_crudas"):
                rows = _manual_paste_parse(raw); _ingest_manual(rows, destino="crudas"); st.rerun()
        with colp2:
            if st.button("📤 Ingresar a Cola (procesar)", use_container_width=True, disabled=not raw.strip(), key="exp_paste_cola"):
                rows = _manual_paste_parse(raw); _ingest_manual(rows,destino="cola"); st.rerun()
        with colp3:
            st.download_button("⬇️ Descargar noticias actuales (CSV)",
                df.to_csv(index=False).encode("utf-8"),
                file_name=f"noticias_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}Z.csv",
                mime="text/csv", use_container_width=True, key="exp_descarga")
...
def _ui_procesar():
    ...
    with c1:
        if st.button("🔤 Abrir Gematría", use_container_width=True, key="proc_gem"):
            _router_go("gematria")
    with c2:
        if st.button("🌀 Abrir Subliminal", use_container_width=True, key="proc_sub"):
            _router_go("subliminal")
    with c3:
        if st.button("📥 Ver cola (pendientes)", use_container_width=True, key="proc_cola"):
            st.session_state["_show_queue"] = True
            st.rerun()
...
def render_noticias():
    ...
    # Pestañas internas estables (evita cambios bruscos en el DOM)
    tabs = st.tabs([
        "🗞️ Crudas",
        "🔥 Filtradas",
        "⚙️ Procesar",
